"""Tests for the :mod:`by_getattr` package."""
...
def test_version_attribute() -> None:
    actual_version = by_getattr.__version__
    assert actual_version == _EXPECTED_VERSION


def test_version_metadata() -> None:
    actual_version = _version(by_getattr.__name__)
    assert actual_version == _EXPECTED_VERSION
...
def test_nonexistent_attribute() -> None:
    with pytest.raises(
        AttributeError,
        match=r"\Amodule 'by_getattr' has no attribute 'nonexistent'\Z",
    ):
        by_getattr.nonexistent  # type: ignore[attr-defined]


def test_dir_lists_common_dunders() -> None:
    common = {'__doc__', '__loader__', '__name__', '__package__', '__spec__'}
    assert common <= set(dir(by_getattr))


def test_dir_lists_hello_attribute() -> None:
    assert "hello" in dir(by_getattr)


def test_dir_lists_version_attribute() -> None:
    assert "__version__" in dir(by_getattr)
